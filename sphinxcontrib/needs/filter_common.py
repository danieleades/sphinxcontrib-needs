...
from sphinxcontrib.needs.utils import logger, merge_two_dicts, status_sorter
...
def procces_filters(all_needs, current_needlist):
    ...
    if not filter_code or filter_code.isspace():
        for need_info in all_needs_incl_parts:
            ...
            if (
                current_needlist["status"] is None
                or len(current_needlist["status"]) == 0
            ):
                # Filtering for status was not requested
                status_filter_passed = True
            elif (
                need_info["status"] is not None
    ...
    filter_list[current_needlist["target_node"]] = {
        ...
        "filter": current_needlist["filter"]
        if current_needlist["filter"] is not None
        else "",
