...
from sphinxcontrib.plantuml import (  # Need for plantuml filename calculation
    generate_name,
)
...
class NeedflowDirective(FilterBase):
    ...
    def run(self):
        ...
        env.need_all_needflows[targetid] = {
            ...
            "show_legend": True
            if self.options.get("show_legend", False) is None
            else False,
            "show_link_names": True
            if self.options.get("show_link_names", False) is None
            else False,
...
def process_needflow(app, doctree, fromdocname):
    ...
    for node in doctree.traverse(Needflow):
        ...
        for need_info in found_needs:
            ...
            if not (
                ...
                for need_part in valid_need_parts:
                    ...
                    if (
                        current_needflow["highlight"] is not None
                        and current_needflow["highlight"] != ""
                        and filter_single_need(
            ...
            for link_type in link_types:
                ...
                for link in need_info[link_type["option"]]:
                    ...
                    if "." in link or "." in need_info["id_complete"]:
                        ...
                        if (
                            "style_part" in link_type.keys()
                            and link_type["style_part"] is not None
                            and len(link_type["style_part"]) > 0
                        ):
                        ...
                        if (
                            "style" in link_type.keys()
                            and link_type["style"] is not None
                            and len(link_type["style"]) > 0
                        ):
                    ...
                    if (
                        "style_start" in link_type.keys()
                        and link_type["style_start"] is not None
                        and len(link_type["style_start"]) > 0
                    ):
                    ...
                    if (
                        "style_end" in link_type.keys()
                        and link_type["style_end"] is not None
                        and len(link_type["style_end"]) > 0
                    ):
        ...
        if (
            current_needflow["align"] is not None
            and len(current_needflow["align"]) != ""
        ):
        ...
        if (
            current_needflow["caption"] is not None
            and len(current_needflow["caption"]) != ""
        ):
