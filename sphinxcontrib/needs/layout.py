...
class LayoutHandler:
    ...
    def meta_links_all(self, prefix="", postfix="", exclude=None):
        ...
        for link_type in self.app.config.needs_extra_links:
            ...
            if (
                self.need[type_key] is not None
                and len(self.need[type_key]) > 0
                and type_key not in exclude
            ):
