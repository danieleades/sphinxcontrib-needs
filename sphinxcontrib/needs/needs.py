...
def setup(app):
    ...
    app.add_role(
        "need",
        XRefRole(nodeclass=Need_ref, innernodeclass=nodes.emphasis, warn_dangling=True),
    )

    app.add_role(
        "need_incoming",
        XRefRole(
            nodeclass=Need_incoming, innernodeclass=nodes.emphasis, warn_dangling=True
        ),
    )
    ...
    app.add_role(
        "need_outgoing",
        XRefRole(
            nodeclass=Need_outgoing, innernodeclass=nodes.emphasis, warn_dangling=True
        ),
    )
...
def load_config(app, *args):
    ...
    extra_options = getattr(
        app.config, "needs_extra_options", app.config.needs_extra_options
    )

    # Get extra links and create a dictionary of needed options.
    extra_links_raw = getattr(
        app.config, "needs_extra_links", app.config.needs_extra_links
    )
    extra_links = {}
    for extra_link in extra_links_raw:
        extra_links[extra_link["option"]] = directives.unchanged

    title_optional = getattr(
        app.config, "needs_title_optional", app.config.needs_title_optional
    )
    title_from_content = getattr(
        app.config, "needs_title_from_content", app.config.needs_title_from_content
    )
